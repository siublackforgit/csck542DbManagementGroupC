...
def _get_search_paths() -> List[str]:
    ...
    try:
        ...
        # Fallback paths
        search_paths.append(ui_dir)          # app/ui/
        search_paths.append(app_dir)         # app/
        search_paths.append(os.getcwd())     # Current working directory
...
def load_app_icons() -> Dict[str, Optional[tk.PhotoImage]]:
    ...
    icons = {
        "header": load_single_icon(
            ...
            subsample=(6, 6)  # Resize to 1/6 original size
        ),
        "run": load_single_icon(
            icon_names=["Run Query.png", "Run_Query.png", "run_query.png"],
            subsample=(14, 14)  # Resize to 1/14 original size
        ),
